...
def ejecutar_ffmpeg(archivo_video, output_path):
    cmd = f"ffmpeg -i {archivo_video} -i /static/watermark.png -filter_complex \"[0:v][1:v] blend=all_mode='overlay':all_opacity=0.7[v]; [v]format=yuv420p\" -c:v h264_nvenc -b:v 1M -c:a copy /destino/{output_path}"
    os.system(cmd)
    print(output_path)
